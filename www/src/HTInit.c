...
**	@(#) $Id: HTInit.c,v 2.100 2000/12/18 17:00:56 kahan Exp $
**
...
#else
    err = _dupenv_s(&env, &len, "DISPLAY");
    if (!err && env != NULL) {	/* Must have X11 */
        FREE(env);
...
PUBLIC void HTIconInit (const char * url_prefix)
{
	...
	{
	    char * str = NULL;
        int size = strlen(W3C_ICONS) + 4;
        if ((str = (char *) HT_MALLOC(size)) == NULL) {
            HT_OUTOFMEM("HTIconInit");
        }
	    strcpy_s(str, size, W3C_ICONS);
        if (*(str + strlen(str) - 1) != '/') {
            strcat_s(str, size, "/");
        }
	    strcat_s(str, size, "*");
