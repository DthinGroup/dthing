...
PUBLIC HTParentAnchor * HTHomeAnchor (void)
{
    ...
    int len = 0;
    errno_t err = 0;
    BOOLEAN freehome = FALSE;

    err = _dupenv_s(&home, &len, LOGICAL_DEFAULT);
    if (err != 0 || home == NULL) {
        home = NULL;
    } else {
        freehome = TRUE;
    ...
    if (!my_home_document) {
        ...
        if (home) {
            int size = strlen(home)+1+ strlen(PERSONAL_DEFAULT)+1;
            if ((my_home_document = (char  *) HT_MALLOC(size)) == NULL) {
                HT_OUTOFMEM("HTLocalName");
            }
            sprintf_s(my_home_document, size, "%s/%s", home, PERSONAL_DEFAULT);
            fopen_s(&fp, my_home_document, "r");
...
PUBLIC HTParentAnchor * HTTmpAnchor (HTUserProfile * up)
{
	...
	char * result;
    int size = strlen(tmpurl)+20;
    if (!(result = (char *) HT_MALLOC(size))) {
	    HT_OUTOFMEM("HTTmpAnchor");
    }
#ifdef HAVE_LONG_TIME_T
	sprintf_s(result, size, "%s.%ld.%d", tmpurl, t, offset++);
#else
	sprintf_s(result, size, "%s.%d.%d", tmpurl, t, offset++);
#endif
