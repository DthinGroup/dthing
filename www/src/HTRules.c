    ...
    while ((pres = (HTRule *) HTList_nextObject(list))) {
        ...
        case HT_Map:
            if (!pres->replace) {			       /* No replace */
                ...
                int size = strlen(pres->replace) + strlen(rest);
                if ((replace = (char *) HT_MALLOC(size)) == NULL) {
                    HT_OUTOFMEM("HTRule_translate");
                }
                strcpy_s(replace, size, pres->replace);
                strcpy_s(replace + pres->insert, size - pres->insert, rest);
...
PUBLIC BOOL HTRule_parseLine (HTList * list, const char * config)
{
	...
	status = ptr ? sscanf_s(ptr, "%lf", &quality) : 0;
	...
	double quality, secs, secs_per_byte;
        status = ptr ? sscanf_s(ptr,"%lf%lf%lf",&quality,&secs,&secs_per_byte):0;
	...
	int port = 0;
        status = ptr ? sscanf_s(ptr, "%d", &port) : 0;
