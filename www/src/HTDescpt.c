...
PUBLIC HTList * HTReadDescriptions (char * dirname)
{
    ...
    size = strlen(dirname) + strlen(HTDescriptionFile) + 2;
    if ((name = (char *) HT_MALLOC(size)) == NULL) {
        HT_OUTOFMEM("HTReadDescriptions");
    }

    sprintf_s(name, size, "%s/%s", dirname, HTDescriptionFile);
    fopen_s(&fp, name, "r");
    if (!fp) {
    ...
    while (fgets(buf, MAX_LINE_LEN, fp)) {
        ...
        if (t && d && *t && *d) {
            char * stuff;
            size = strlen(t) + strlen(d) + 2;
            if ((stuff = (char  *) HT_MALLOC(size)) == NULL) {
                HT_OUTOFMEM("HTDirReadDescriptions");
            }
            sprintf_s(stuff, size, "%s %s", t, d);
...
#define PEEK_BUF_SIZE 200
    ...
    sprintf_s(name, size, "%s/%s", dirname, filename);
