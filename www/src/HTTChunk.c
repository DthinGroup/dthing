...
PRIVATE int HTChunkEncode_block (HTStream * me, const char * b, int l)
{
    ...
    if (me->param) {
	if (me->total)
	    sprintf_s(chunky, HTChunk_allocated(me->buf), "%c%c%x %s %c%c", CR, LF, l, me->param, CR, LF);
	else
	    sprintf_s(chunky, HTChunk_allocated(me->buf), "%x %s %c%c", l, me->param, CR, LF);
    } else {
	if (me->total)
	    sprintf_s(chunky, HTChunk_allocated(me->buf), "%c%c%x%c%c", CR, LF, l, CR, LF);
	else
	    sprintf_s(chunky, HTChunk_allocated(me->buf), "%x%c%c", l, CR, LF);
    }
    me->total += l;
