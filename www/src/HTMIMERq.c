...
PRIVATE int MIMEMakeRequest (HTStream * me, HTRequest * request)
{
    ...
    *crlf = CR; *(crlf+1) = LF; *(crlf+2) = '\0';
	...
	if (!HTFormat_isUnityTransfer(cte)) {
	    sprintf_s(linebuf, sizeof(linebuf), "Content-Transfer-Encoding: %s%c%c",
	...
	if (EntityMask & HT_E_CONTENT_LENGTH) {
		...
		sprintf_s(linebuf, sizeof(linebuf), "Content-Length: %ld%c%c",
			entity->content_length, CR, LF);
		PUTBLOCK(linebuf, (int) strlen(linebuf));
	    } else {
		transfer_coding = YES;
    	        sprintf_s(linebuf, sizeof(linebuf), "Transfer-Encoding: %s%c%c",
	...
	sprintf_s(linebuf, sizeof(linebuf), "Derived-From: %s%c%c", entity->derived_from,
		CR, LF);
	PUTBLOCK(linebuf, (int) strlen(linebuf));
    }
    if (EntityMask & HT_E_EXPIRES) {
	if (entity->expires != -1) {
	    sprintf_s(linebuf, sizeof(linebuf), "Expires: %s%c%c",
	...
	if (entity->last_modified != -1) {
	    sprintf_s(linebuf, sizeof(linebuf), "Last-Modified: %s%c%c",
	...
	if (link && (linktype = HTLink_type(link))) {
		...
		PUTS(rel_dst);
		PUTS("\"");
		sprintf_s(linebuf, sizeof(linebuf), ";rel=\"%s\"", HTAtom_name(linktype));
		PUTBLOCK(linebuf, (int) strlen(linebuf));
		HT_FREE(rel_dst);
		HT_FREE(dst);
	    }
	...
	sprintf_s(linebuf, sizeof(linebuf), "Title: %s%c%c", entity->title, CR, LF);
	PUTBLOCK(linebuf, (int) strlen(linebuf));
    }
	...
	sprintf_s(linebuf, sizeof(linebuf), "Content-Version: %s%c%c", entity->version, CR, LF);
	PUTBLOCK(linebuf, (int) strlen(linebuf));
    }
    if (me->endHeader) {
	sprintf_s(linebuf, sizeof(linebuf), "%c%c", CR, LF);	   /* Blank line means "end" */
	PUTBLOCK(linebuf, (int) strlen(linebuf));
    }
