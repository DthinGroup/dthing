...
PRIVATE int HTTPGenMake (HTStream * me, HTRequest * request)
{
    ...
    *crlf = CR; *(crlf+1) = LF; *(crlf+2) = '\0';
	...
	sprintf_s(linebuf, sizeof(linebuf), "Date: %s%c%c", HTDateTimeStr(&local, NO), CR,LF);
	...
	sprintf_s(linebuf, sizeof(linebuf), "Pragma: %s%c%c", "no-cache", CR, LF);
	PUTBLOCK(linebuf, (int) strlen(linebuf));
    }
    if (gen_mask & HT_G_MESSAGE_ID) {
	const char *msgid = HTMessageIdStr(HTRequest_userProfile(request));
	if (msgid) {
	    sprintf_s(linebuf, sizeof(linebuf), "Message-ID: %s%c%c", msgid, CR, LF);
	    PUTBLOCK(linebuf, (int) strlen(linebuf));
	}
    }
    if (gen_mask & HT_G_MIME) {
	sprintf_s(linebuf, sizeof(linebuf), "MIME-Version: %s%c%c", MIME_VERSION, CR, LF);
	PUTBLOCK(linebuf, (int) strlen(linebuf));
    }
    ...
    if (!HTMethod_hasEntity(HTRequest_method(request)))
    {
        ...
        if (body && *body) {
            if ( bodyLength>0 ) {
                ...
                sprintf_s(linebuf, sizeof(linebuf), "Content-Length: %ld%c%c", bodyLength, CR,LF);
                PUTBLOCK(linebuf, (int) strlen(linebuf));
            }
            if ( bodyFormat != NULL ) {
