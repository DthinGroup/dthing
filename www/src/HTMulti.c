...
**	@(#) $Id: HTMulti.c,v 2.36 2000/01/06 10:48:50 kahan Exp $
**
** History:
	...
	*multi = 0;
	...
	if ((int) strlen(dirbuf->d_name) >= baselen) {
		...
		if (HTBind_getFormat(dirbuf->d_name,
				     ...
				     &cd->quality)) {
		    if (cd->content_type) {
                int size = strlen(dirname) + 2 + strlen(dirbuf->d_name);
                if ((cd->filename = (char *) HT_MALLOC(size)) == NULL)
                    HT_OUTOFMEM("dir_matches");
                sprintf_s(cd->filename, size, "%s/%s", dirname, dirbuf->d_name);
...
PRIVATE char * get_best_welcome (char * path)
{
    ...
    if (best_welcome) {
        char * welcome;
        int size = strlen(path) + strlen(best_welcome)+2;
        if ((welcome = (char *) HT_MALLOC(size)) == NULL)
            HT_OUTOFMEM("get_best_welcome");
        sprintf_s(welcome, size, "%s%s%s", path, last ? "" : "/", best_welcome);
