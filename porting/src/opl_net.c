...
static void net_pdpCallbck(uint32 id, uint32 argc, void *argv)
{
    ...
    NET_Trace("==RMT==net_pdpCallbck - EV_MN_APP_SET_PDP_CONTEXT_CNF_F=%d", EV_MN_APP_SET_PDP_CONTEXT_CNF_F);
    NET_Trace("==RMT==net_pdpCallbck - INFO: id (%d)  argc(%d)", id, argc);
    NET_Trace("==RMT==net_pdpCallbck - INFO: signal_ptr->result (%d)", signal_ptr->result);
    NET_Trace("==RMT==net_pdpCallbck - INFO: signal_ptr->dual_sys (%d)", signal_ptr->dual_sys);
    NET_Trace("==RMT==net_pdpCallbck - INFO: signal_ptr->pdp_id (%d)", signal_ptr->pdp_id);
    NET_Trace("==RMT==net_pdpCallbck - INFO: signal_ptr->nsapi (%d)", signal_ptr->nsapi);
    ...
    if(EV_MN_APP_ACTIVATE_PDP_CONTEXT_CNF_F == argc)
    {
        if(MN_GPRS_ERR_SUCCESS == signal_ptr->result || MN_GPRS_ERR_PDP_CONTEXT_ACTIVATED == signal_ptr->result )
        {
            ...
            NET_Trace("==RMT== net_pdpCallbck()-------gprs ok--------isActived=%d netId=%d",isActived,netId);
            isActived = TRUE;
        }
        else if(MN_GPRS_ERR_RETRYING == signal_ptr->result)
        {
            NET_Trace("==RMT== net_pdpCallbck MN_GPRS_ERR_RETRYING == signal_ptr->result retrytimes=%d",retrytimes);
            SCI_Sleep(5000);
            if(retrytimes < 5)
            {
                ...
                {
                    NET_Trace("==RMT== net_pdpCallbck 2  MNGPRS_SetAndActivePdpContextEx error");
                }
                else
                {
                    NET_Trace("==RMT== net_pdpCallbck 2 MNGPRS_SetAndActivePdpContextEx waiting for gprs rsp");
                }
            }
            retrytimes++;
        }
        else
        {
            ...
            if(retrytimes < 5)
            {
                ...
                if(ERR_MNGPRS_NO_ERR != MNGPRS_ActivatePdpContextEx(MN_DUAL_SYS_1, TRUE, pdp_id_arr, BROWSER_E, MN_GSM_PREFER))
                {
                    NET_Trace("==RMT== net_pdpCallbck 3  MNGPRS_SetAndActivePdpContextEx error");
                }
                else
                {
                    NET_Trace("==RMT== net_pdpCallbck 3 MNGPRS_SetAndActivePdpContextEx waiting for gprs rsp");
                }
            }
            retrytimes++;
            NET_Trace("==RMT== net_pdpCallbck()-------gprs err--------");
        }
    }
    else if(EV_MN_APP_DEACTIVATE_PDP_CONTEXT_CNF_F == argc)
    {
        NET_Trace("==RMT== net_pdpCallbck()-------EV_MN_APP_DEACTIVATE_PDP_CONTEXT_CNF_F in--------signal_ptr->result=&d",signal_ptr->result);
    ...
    {
        NET_Trace("==RMT== net_pdpCallbck()-------not EV_MN_APP_ACTIVATE_PDP_CONTEXT_CNF_F event--------");
    }
}
...
int net_pdpactive()
{
    ...
    NET_Trace("==RMT==net_pdpactive - MNGPRS_SetPdpContextEx  ERR=%d",err);

    err = MNGPRS_SetPdpContextPcoEx(MN_DUAL_SYS_1,3,user,passwd);
    NET_Trace("==RMT==net_pdpactive - MNGPRS_SetPdpContextPcoEx  ERR=%d",err);

    //NET_Trace("==RMT==net_pdpactive - ERR_MNGPRS_CALL_FAILURE=%d ",ERR_MNGPRS_CALL_FAILURE);
    err = MNGPRS_ActivatePdpContextEx(MN_DUAL_SYS_1, TRUE, pdp_id_arr, BROWSER_E, MN_GSM_PREFER);
    NET_Trace("==RMT==net_pdpactive  MNGPRS_ActivatePdpContextEx ERR=%d",err);

    while(times-->0)
    {
        if(isActived == TRUE)
        {
            NET_Trace("==RMT== while true isActived == 1");
            break;
        }
        SCI_Sleep(1000);
    }
...
int  net_activeNetwowrk()
{
    ...
    NET_Trace("==RMT==  MNPHONE_StartupPsEx -  start");
    err = MNPHONE_StartupPsEx(MN_DUAL_SYS_1,MN_GMMREG_RAT_GPRS);
    NET_Trace("==RMT==  MNPHONE_StartupPsEx -  end  ERR=%d",err);
    SCI_Sleep(4*1000);

    NET_Trace("==RMT==  MNPHONE_GprsAttachEx -  start");
    err = MNPHONE_GprsAttachEx(MN_DUAL_SYS_1,MN_PHONE_GPRS_ATTACH);
    NET_Trace("==RMT== - MNPHONE_GprsAttachEx   end  ERR=%d",err);
    SCI_Sleep(4*1000);

    NET_Trace("==RMT==net_activeNetwowrk - SCI_RegisterMsg  net_pdpCallbck");
...
{
    ERR_MNGPRS_CODE_E err = ERR_MNGPRS_NO_ERR;
    NET_Trace("==RMT== net_deactiveNetwowrk MNGPRS_ResetAndDeactivePdpContextEx--------");
    err = MNGPRS_ResetAndDeactivePdpContextEx(MN_DUAL_SYS_1);
    NET_Trace("==RMT== net_deactiveNetwowrk MNGPRS_ResetAndDeactivePdpContextEx--------err=%d",err);
}
#endif
...
{    
    ...
    NET_Trace("call Opl_net_activate\n");
    net_activeNetwowrk();
    NET_Trace("call Opl_net_activate over\n");
#endif
}
...
int Opl_net_isActivated(void)
{
    ...
    return isActived;
#elif defined(ARCH_ARM_SPD)
    NET_Trace("Opl_net_isActivated:%d",isActived);
    return isActived;
#endif    
}
