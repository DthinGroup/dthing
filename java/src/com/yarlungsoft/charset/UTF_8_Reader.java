...
public class UTF_8_Reader extends StreamReader {
     ...
     * @return the number of converted characters with -1 indicating end of stream.
     * @throws IOException if the input stream could not be read for the raw unconverted character
     */
    public int read(char cbuf[], int off, int len) throws IOException {
        ...
        while (0 < len && cont_flag) {
            ...
            /*
             * to avoid blocking in network, we treat less read than we required as EOF and will
             * never try to read more
             */
    ...
    /**
     * Tell whether this reader supports the mark() operation. The UTF-8 implementation always
     * returns false because it does not support mark().
     *
     * @return false
     */
    public boolean markSupported() {
        /*
         * For readers mark() is in characters, since UTF-8 character are variable length, so we
         * can't just forward this to the underlying byte InputStream like other readers do.
     ...
     * @throws IOException for all calls to this method because marking is not supported for UTF8
     *         readers
     */
     ...
     * <p/>
     * This method is only used by our internal method
     * {@link Helper#byteToCharArray(byte[], int, int, String)} to know how much to allocate before
     * using a reader. If we encounter bad encoding we should return a count that includes that
     * character so the reader will throw an {@link IOException}.
     *
