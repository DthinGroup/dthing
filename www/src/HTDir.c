...
**	@(#) $Id: HTDir.c,v 2.23 2000/06/27 10:06:59 kahan Exp $
**
...
PRIVATE BOOL HTDirNode_print (HTDir *dir, HTDirNode *node)
{
	...
	if (dir->base) {
	    ...
	    char *full;
        int size = strlen(escaped)+strlen(dir->base)+1;
        if ((full = (char *) HT_MALLOC(size)) == NULL) {
            HT_OUTOFMEM("HTDirNode_print");
        }
	    strcpy_s(full, size, dir->base);
	    strcat_s(full, size, escaped);
...
**	Returns YES if OK, else NO
*/
PUBLIC BOOL HTDir_addElement (HTDir *dir, char *name, char *date, char *size, HTFileMode mode)
{
    HTDirNode *node = HTDirNode_new();
    int sz = 0;
    if (!dir || !name) return NO;
    sz = strlen(name) + 2;
    if ((node->fname = (char *) HT_MALLOC(sz)) == NULL) {
        HT_OUTOFMEM("HTDir_addElement");
    }
    strcpy_s(node->fname, sz, name);   /* Mandatory */
...
PUBLIC BOOL HTDir_free (HTDir * dir)
{
	...
	if (!dir->size)
	    ...
	    char buffer[20];
	    sprintf_s(buffer, sizeof(buffer), "%u files", dir->size);
	    PUTS(buffer);
	}
	END(HTML_PRE);
	END(HTML_BODY);
