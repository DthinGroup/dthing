...
**	@(#) $Id: HTTPReq.c,v 2.62 2001/08/30 13:41:03 kahan Exp $
**
...
PRIVATE int HTTPMakeRequest (HTStream * me, HTRequest * request)
{
	...
	*/
	if (format == WWW_PRESENT) {
		...
		if ((!list && ((cur = HTFormat_conversion()) != NULL)) ||
			...
			if (pres->rep_out==WWW_PRESENT && pres->quality<=1.0) {
				...
				sprintf_s(qstr, sizeof(qstr), ";q=%1.1f", pres->quality);
				PUTS(qstr);
			    }
			}
		    }
	...
	for (list=0; list<2; list++) {
		...
		while ((pres = (HTCoding *) HTList_nextObject(cur))) {
			...
			sprintf_s(qstr, sizeof(qstr), ";q=%1.1f", quality);
			PUTS(qstr);
		    }
		}
	    }
    ...
    */
	...
	if (hops >= 0) {
	    sprintf_s(qstr, sizeof(qstr), "%d", hops);
	    PUTS("Max-Forwards: ");
	    PUTS(qstr);
	    PUTBLOCK(crlf, 2);
	}
