...
PRIVATE char * cache_index_name (const char * cache_root)
{
    if (cache_root) {
        char * location = NULL;
        int size = strlen(cache_root) + strlen(HT_CACHE_INDEX) + 1;
        if ((location = (char *) HT_MALLOC(size)) == NULL) {
            HT_OUTOFMEM("cache_index_name");
        }
        strcpy_s(location, size, cache_root);
        strcat_s(location, size, HT_CACHE_INDEX);
        return location;
    }
    return NULL;
}
...
PRIVATE BOOL HTCacheIndex_parseLine (char * line)
{
	...
	**  problems.
	*/
	if (sscanf_s(line, "%ld %ld %ld %c %d %d %ld %ld %ld %c",
#else
	if (sscanf_s(line, "%d %d %ld %c %d %d %d %d %d %c",
#endif
		   &cache->lm,
		   &cache->expires,
		   &cache->size,
...
PRIVATE BOOL HTCacheMode_setRoot (const char * cache_root)
{
    if (cache_root) {
        ...
        */
        ...
        int len = 0;
        errno_t err = 0;
        BOOLEAN freecr = FALSE;

        err = _dupenv_s(&cr, &len, "WWW_CACHE");
        if (err != 0 || cr == NULL) {
            cr = NULL;
        } else {
            freecr = TRUE;
...
PRIVATE BOOL HTCache_getSingleUserLock (const char * root)
{
    if (root && !locked_open_file) {
        FILE * fp;
        char * location = NULL;
        int size = strlen(root) + strlen(HT_CACHE_LOCK) + 1;
        if ((location = (char *) HT_MALLOC(size)) == NULL) {
            HT_OUTOFMEM("HTCache_getLock");
        }
        strcpy_s(location, size, root);
        strcat_s(location, size, HT_CACHE_LOCK);
...
PRIVATE BOOL HTCache_createLocation (HTCache * me)
{
	...
	char * path = NULL;
	struct stat stat_info;
    int size = strlen(HTCacheRoot) + 10;
    if ((path = (char *) HT_MALLOC(size)) == NULL) {
	...
	*/
	sprintf_s(path, size, "%s%d", HTCacheRoot, me->hash);
...
PRIVATE char * HTCache_metaLocation (HTCache * cache)
{
    ...
    if (cache && cache->cachename && *cache->cachename) {
        int size = strlen(cache->cachename) + strlen(HT_CACHE_META) + 5;
        if ((local = (char *) HT_MALLOC(size)) == NULL) {
            HT_OUTOFMEM("HTCache_metaLocation");
        }
        sprintf_s(local, size, "%s%s", cache->cachename, HT_CACHE_META);
    }
    return local;
}
...
PUBLIC HTReload HTCache_isFresh (HTCache * cache, HTRequest * request)
{
	...
	*/
	if (cache->range) {
	    char buf[20];
	    sprintf_s(buf, sizeof(buf), "%ld-", cache->size);
...
PRIVATE HTStream * HTCacheStream (HTRequest * request, BOOL append)
{
    ...
    */
    fopen_s(&fp, cache->cachename, append ? "ab" : "wb");
    if (fp == NULL) {
        HTTRACE(CACHE_TRACE, "Cache....... Can't open `%s\' for writing\n" _ cache->cachename);
