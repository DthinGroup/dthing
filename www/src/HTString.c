...
PUBLIC char * HTSACopy (char ** dest, const char * src)
{
  ...
  if (! src)
    *dest = NULL;
    ...
    if ((*dest  = (char  *) HT_MALLOC(size)) == NULL)
...
PUBLIC char * HTSACat (char ** dest, const char * src)
{
  if (src && *src) {
    ...
    if (*dest) {
      ...
      if ((*dest  = (char  *) HT_REALLOC(*dest, length + size)) == NULL)
          HT_OUTOFMEM("HTSACat");
      strcpy_s(*dest + length, size, src);
    } else {
      if ((*dest  = (char  *) HT_MALLOC(size)) == NULL)
          HT_OUTOFMEM("HTSACat");
      strcpy_s(*dest, size, src);
...
PUBLIC char * StrAllocMCopy (char ** dest, ...)
{
    ...
    remainSize = needed + 1;
	if ((*dest = (char *) HT_MALLOC(remainSize)) == NULL)
...
PUBLIC char * StrAllocMCat (char ** dest, ...)
{
	...
	if (*dest) {
	    ...
	    if ((*dest = (char *) HT_REALLOC(*dest, dest_len + remainSize)) == NULL)
