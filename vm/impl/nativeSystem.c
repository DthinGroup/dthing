...
void Java_java_lang_System_arraycopy(const u4* args, JValue* pResult) {
    ...
    /* avoid int overflow */
    if (srcPos < 0 || dstPos < 0 || length < 0 || srcPos > (int) srcArray->length - length
    ...
    if (srcPrim || dstPrim) {
        ...
        DVMTraceInf("arraycopy prim[%c] dst=%p %d src=%p %d len=%d\n", srcType, dstArray->contents,
            dstPos, srcArray->contents, srcPos, length);

        switch (srcType) {
        case 'B':
        case 'Z':
            /* 1 byte per element */
            CRTL_memmove((u1*) dstArray->contents + dstPos, (const u1*) srcArray->contents + srcPos,
                length);
            break;
        case 'C':
        case 'S':
            ...
            move16((u1*) dstArray->contents + dstPos * 2,
                (const u1*) srcArray->contents + srcPos * 2, length * 2);
            break;
        case 'F':
        case 'I':
            /* 4 bytes per element */
            move32((u1*) dstArray->contents + dstPos * 4,
                (const u1*) srcArray->contents + srcPos * 4, length * 4);
            break;
        case 'D':
        case 'J':
            /*
            ...
            move32((u1*) dstArray->contents + dstPos * 8,
                (const u1*) srcArray->contents + srcPos * 8, length * 8);
            break;
        ...
        if (srcClass->arrayDim == dstClass->arrayDim && dvmInstanceof(srcClass, dstClass)) {
            ...
            DVMTraceInf("arraycopy ref dst=%p %d src=%p %d len=%d\n", dstArray->contents,
                dstPos * width, srcArray->contents, srcPos * width, length * width);
            move32((u1*) dstArray->contents + dstPos * width,
                (const u1*) srcArray->contents + srcPos * width, length * width);
            ...
            DVMTraceInf("arraycopy iref dst=%p %d src=%p %d count=%d of %d\n", dstArray->contents,
                dstPos * width, srcArray->contents, srcPos * width, copyCount, length);
            move32((u1*) dstArray->contents + dstPos * width,
...
void Java_com_yarlungsoft_util_SystemInputStream_readN(const u4* args, JValue* pResult) {
    // TODO : implement
    DVMTraceInf("Java_com_yarlungsoft_util_SystemInputStream_readN is not implemented..\n");
    RETURN_INT(0);
}
...
void Java_com_yarlungsoft_util_SystemPrintStream_flush(const u4* args, JValue* pResult) {
    // TODO : implement
    DVMTraceInf("Java_com_yarlungsoft_util_SystemPrintStream_flush is not implemented..\n");
    RETURN_VOID();
}
