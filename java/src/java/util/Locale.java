...
public final class Locale implements Cloneable, Serializable {
    ...
    /**
     * Built-in available locales: en, en_GB, en_US, zh, zh_CN
    ...
    private static final String[] isoCountries = new String[] {
        ROOT.getCountry(), UK.getCountry(), US.getCountry(), SIMPLIFIED_CHINESE.getCountry()
    };
    private static final String[] isoLanguages = new String[] {
        ROOT.getLanguage(), UK.getLanguage(), US.getLanguage(), SIMPLIFIED_CHINESE.getLanguage()
    };
    ...
    public String getDisplayCountry(Locale locale) {
        ...
        // TODO: Implement native method to replace the following hard coding.
        if (countryCode.equals(UK.countryCode)) {
            if (locale.languageCode.equals(CHINESE.languageCode)) {
                return "英国";
            }
            return "United Kingdom of Great Britain & N. Ireland";
        }
        if (countryCode.equals(US.countryCode)) {
            if (locale.languageCode.equals(CHINESE.languageCode)) {
                return "美国";
            }
            return "United States of America";
        }
        if (countryCode.equals(SIMPLIFIED_CHINESE.countryCode)) {
            if (locale.languageCode.equals(CHINESE.languageCode)) {
                return "中国";
            }
    ...
    public String getDisplayLanguage(Locale locale) {
        ...
        // TODO: Implement native method to replace the following hard coding.
        if (languageCode.equals(ENGLISH.languageCode)) {
            if (locale.languageCode.equals(CHINESE.languageCode)) {
                return "英语";
            }
            return "English";
        }
        if (languageCode.equals(CHINESE.languageCode)) {
            if (locale.languageCode.equals(CHINESE.languageCode)) {
                return "中文";
            }
            return "Chinese";
    ...
    public String getDisplayName(Locale locale) {
        ...
        if (!TextUtils.isEmpty(languageCode)) {
            ...
            buffer.append(TextUtils.isEmpty(displayLanguage) ? languageCode : displayLanguage);
            ++count;
        }
        if (!TextUtils.isEmpty(countryCode)) {
            if (count == 1) {
            ...
            buffer.append(TextUtils.isEmpty(displayCountry) ? countryCode : displayCountry);
            ++count;
        }
        if (!TextUtils.isEmpty(variantCode)) {
            if (count == 1) {
                buffer.append(" (");
            } else if (count == 2) {
                buffer.append(",");
            ...
            buffer.append(TextUtils.isEmpty(displayVariant) ? variantCode : displayVariant);
            ++count;
        }
        if (count > 1) {
            buffer.append(")");
    ...
    public String getISO3Language() {
        if (languageCode.length() == 0 || languageCode.length() == 3) {
            return languageCode;
        }
        // TODO: Implement native method to replace the following hard coding.
        if (languageCode.equals(ENGLISH.languageCode)) {
