...
PRIVATE ms_t HTMemLog_addTime(void)
{
    ...
    int len = sprintf_s(buff, sizeof(buff), "%lu", ms);
...
PUBLIC int HTMemLog_open (char * logName, size_t size, BOOL keepOpen)
{
    ...
    _sopen_s(&LogFd, LogName, OPEN_FLAGS, _SH_DENYWR, 0666);
    if (LogFd == -1)
        return HT_ERROR;
    if (!KeepOpen)
        CLOSE(LogFd);
...
PUBLIC int HTMemLog_flush(void)
{
    ...
    if (LogLen) {
        if (!KeepOpen) {
            _sopen_s(&LogFd, LogName, O_WRONLY|O_CREAT|O_APPEND, _SH_DENYWR, 0666);
            if (LogFd == -1) {
                return HT_ERROR;
            }
        }
...
PUBLIC int HTMemLog_callback (char * data, size_t len, char * fmt, va_list pArgs)
{
    ...
    if (len > 8192)
	len = 8192;
    strncpy_s(buff, sizeof(buff), data, len);
    buff[len] = 0;
    syslog(LOG_DEBUG, "%s\n", buff);
#else /* USE_SYSLOG */
    HTMemLog_addTime();
#ifdef HAVE_LONG_SIZE_T
    ret = sprintf_s(buff, sizeof(buff), " %ld ", len);
#else
    ret = sprintf_s(buff, sizeof(buff), " %d ", len);
#endif
    HTMemLog_add(buff, ret);

    if (fmt) {
      ret = vsprintf_s(buff, sizeof(buff), fmt, pArgs);
      HTMemLog_add(buff, ret);
    }
    HTMemLog_add("\n", 1);
