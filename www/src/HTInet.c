...
#define BUFSIZE 64
    char * msg = NULL;
#ifdef _WINSOCKAPI_
    if ((msg = (char *) HT_MALLOC(BUFSIZE)) == NULL)
	HT_OUTOFMEM("HTErrnoString");
    *msg = '\0';
    sprintf_s(msg, BUFSIZE, "WinSock reported error=%ld", WSAGetLastError());
#else
#ifdef HAVE_STRERROR
	...
	HT_OUTOFMEM("HTErrnoString");
    *msg = '\0';
    sprintf_s(msg, BUFSIZE, "Unix errno=%ld dec, VMS error=%lx hex", errornumber,
	    vaxc$errno);
#else
...
PUBLIC const char * HTInetString (SockA * sin)
{
    ...
    sprintf_s(string, sizeof(string), "%d.%d.%d.%d",
...
PUBLIC int HTParseInet (HTHost * host, char * hostname, HTRequest * request)
{
    ...
    strncpy_s(sin->sdn_nam.n_name, sizeof(sin->sdn_nam.n_name), hostname, sin->sdn_nam.n_len + 1);
...
PUBLIC char * HTGetHostName (void)
{
    ...
    *(name+MAXHOSTNAMELEN) = '\0';
    ...
    {
	FILE *fp;
	if (fqdn==1 && fopen_s(&fp, RESOLV_CONF, "r") == 0) {
	    char buffer[80];
	    *(buffer+79) = '\0';
...
PUBLIC char * HTGetMailAddress (void)
{
  ...
  unsigned int bufSize = sizeof(name);
    ...
    if (!login) {
        err = _dupenv_s(&login, &len, "LOGNAME");
        if (err != 0 || login == NULL) {
            login = NULL;
...
#else
    err = _dupenv_s(&newshost, &len, "NNTPSERVER");
    if (err != 0 || newshost == NULL) {
        newshost = NULL;
    } else {
        freeEnv = TRUE;
    }
...
PUBLIC time_t HTGetTimeZoneOffset (void)
{
    ...
    if (HTTimeZone != -1) {
        return HTTimeZone;           /* Already done */
    }

#ifdef HAVE_TIMEZONE
    cur_t = time(NULL);
#ifdef HT_REENTRANT
    local = (struct tm *) localtime_r(&cur_t, &loctime);
#else
    err = localtime_s(&loctime, &cur_t);
    local = &loctime;
#endif /* HT_REENTRANT */
...
PUBLIC char * HTGetTmpFileName (const char * abs_dir)
{
    ...
    if (abs_dir && *abs_dir) {
        ...
        err = _dupenv_s(&tmpdir, &envLen, "TMPDIR");
        if (err != 0 || tmpdir == NULL) {
            tmpdir = NULL;
        } else {
            freeEnv = TRUE;
        }
        ...
        if (len) {
            ...
            strcpy_s(envtmpdir, len + 8, "TMPDIR=");
            strcpy_s(envtmpdir + 7, len + 1, tmpdir);
            if (freeEnv) {
                FREE(tmpdir);
            }
    ...
    */
    char * offset = NULL;
    int bufsize = (abs_dir ? strlen(abs_dir) : 0) + HT_MAX_TMPNAM + 2;
    if (!(result = (char *) HT_MALLOC(bufsize)))
	HT_OUTOFMEM("HTGetTmpFileName");
    ...
    if (abs_dir && *abs_dir==DIR_SEPARATOR_CHAR) {
#endif /* WWW_MSWINDOWS */
	    strcpy_s(result, bufsize, abs_dir);
    	...
    	tmpnam_r(offset);
#else
	    tmpnam_s(offset, bufsize - (offset - result));
#endif
