...
**	@(#) $Id: HTFile.c,v 1.158 2000/08/10 16:05:39 kahan Exp $
**
...
PRIVATE int HTFile_readDir (HTRequest * request, file_info *file)
{
    ...
    if (dir_access == HT_DIR_FORBID) {
        HTRequest_addError(request, ERR_FATAL, NO, HTERR_FORBIDDEN, NULL, 0, "HTFile_readDir");
        return HT_FORBIDDEN;
    }
    ...
    strcpy_s(fullname, sizeof(fullname), file->local);
    name = fullname + strlen(fullname); /* Point to end of fullname */
    nameSize = sizeof(fullname) - strlen(fullname);

    /* Check if access is enabled */
    if (dir_access == HT_DIR_SELECTIVE) {
        strcpy_s(name, nameSize, DEFAULT_DIR_FILE);
    ...
    if ((dp = opendir(file->local))) {
...
#endif /* HAVE_READDIR_R_2 */
        {
            ...
            strcpy_s(name, nameSize, dirbuf->d_name);
            ...
            if (((mode_t) file_info.st_mode & S_IFMT) == S_IFDIR) {
                ...
                if (dir_show & HT_DS_SIZE)
                    strcpy_s(sizestr, sizeof(sizestr), "-");
            } else {
                mode = HT_IS_FILE;
                if (dir_show & HT_DS_SIZE)
