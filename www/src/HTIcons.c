...
PRIVATE char * prefixed(const char * name, const char * prefix)
{
    ...
    if (name) {
        ...
        int size = len + strlen(name) + 2;
        if ((ret = (char *) HT_MALLOC(size)) == NULL) {
            HT_OUTOFMEM("prefixed");
        }
        if (prefix) {
            strcpy_s(ret, size, prefix);
            if (*prefix && prefix[len-1] != '/') {
                strcat_s(ret, size, "/");
            }
            strcat_s(ret, size, name);
        } else {
            strcpy_s(ret, size, name);
        }
    }
    return ret;
}
...
PUBLIC char * HTIcon_alternative (HTIconNode * node, BOOL brackets)
{
    ...
    if (node) {
        ...
        int size = alt_len + 3;
        if ((p = ret = (char *) HT_MALLOC(size)) == NULL) {
            HT_OUTOFMEM("HTIcon_alt_string");
        }
        *p++ = brackets ? '[' : ' ';
        size--;
        if (node->icon_alt) {
            strcpy_s(p, size, node->icon_alt);
        }
        p += len;
        while (len++ < alt_len) {
            *p++=' ';
