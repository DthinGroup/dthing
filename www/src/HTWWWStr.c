...
#ifdef HAVE_GETPID
    char * address = HTUserProfile_fqdn(up);
#else
    char * address = HTUserProfile_email(up);
#endif /* HAVE_GETPID */
    if (!address) {
        errno_t err = tmpnam_s(tmpbuf, sizeof(tmpbuf));
        address = (err == 0 ? tmpbuf : NULL);
    }
...
#ifdef HAVE_GETPID
    sprintf_s(buf, sizeof(buf), "<%ldZ%ld@%s>", (long)sectime, (long)getpid(), address ? address : "@@@");
#else
    sprintf_s(buf, sizeof(buf), "<%ldZ%s>", sectime, address ? address : "@@@");
#endif /* HAVE_GETPID */
...
#else
            ctime_s(buffer, CTIME_MAX, &t);
		    HTTRACE(CORE_TRACE, "Time string. Delta-time %s parsed to %ld seconds, or in local time: %s" _
			    str _ (long) t _ buffer);
#endif /* HT_REENTRANT */
		} else {
...
PUBLIC const char *HTDateTimeStr (time_t * calendar, BOOL local)
{
    ...
    if (local) {
        ...
        localtime_s(&loctime, calendar);
#endif /* HT_REENTRANT */
	    sprintf_s(buf, sizeof(buf), "%s, %02d %s %04d %02d:%02d:%02d",
    	...
    	gmtime_s(&gmt, calendar);
#endif
	    sprintf_s(buf, sizeof(buf), "%s, %02d %s %04d %02d:%02d:%02d GMT",
		    wkdays[gmt.tm_wday],
		    gmt.tm_mday,
		    months[gmt.tm_mon],
		    gmt.tm_year + 1900,
...
PUBLIC BOOL HTDateDirStr (time_t * time, char * str, int len)
{
    ...
    if (len >= 16) {
        struct tm loctime;
        localtime_s(&loctime, time);
	    sprintf_s(str, len, "%02d-%s-%02d %02d:%02d",
		    loctime.tm_mday,
		    months[loctime.tm_mon],
		    loctime.tm_year % 100,
		    loctime.tm_hour,
...
PUBLIC void HTNumToStr (unsigned long n, char * str, int len)
{
	...
	*str = '\0';
    ...
    if (n < 1000)
	sprintf_s(str, len, "%dK", n>0 ? 1 : 0);
    else if (size + 0.999 < 1000)
	sprintf_s(str, len, "%dK", (int)(size + 0.5));
    else if ((size /= 1024) < 9.9)
	sprintf_s(str, len, "%.1fM", (size + 0.05));
    else if (size < 1000)
	sprintf_s(str, len, "%dM", (int)(size + 0.5));
    else if ((size /= 1024) < 9.9)
	sprintf_s(str, len, "%.1fG", (size + 0.05));
    else
	sprintf_s(str, len, "%dG", (int)(size + 0.5));
}
...
PUBLIC char * HTLocalToWWW (const char * local, const char * access)
{
    ...
    if (local && *local) {
        int size = strlen(local) + 10;
#ifdef VMS
        char * unescaped = NULL;
        if ((unescaped = (char *) HT_MALLOC(size)) == NULL)
            HT_OUTOFMEM("HTLocalToWWW");
        strcpy_s(unescaped, size, scheme);	     /* We get an absolute file name */
	...
	/* convert directory name to Unix-style syntax */
	{
	    ...
	    if (disk) {
		*disk = '\0';
		strcat_s(unescaped, size, "/");
		strcat_s(unescaped, size, local);
	    }
	    if (dir) {
		char *p;
		*dir = '/';	/* Convert leading '[' */
		...
		*p = '\0';	/* Cut on final ']' */
        ...
        if ((unescaped = (char *) HT_MALLOC(size)) == NULL)
            HT_OUTOFMEM("HTLocalToWWW");
        strcpy_s(unescaped, size, scheme);	     /* We get an absolute file name */
        if (strchr(local, ':')) strcat_s(unescaped, size, "/");
        {
            const char *p = local;
