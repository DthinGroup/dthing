...
**	@(#) $Id: HTAABrow.c,v 2.60 2001/03/14 16:25:50 kahan Exp $
**
...
PRIVATE BOOL basic_credentials (HTRequest * request, HTBasic * basic)
{
	...
	char * cleartext = NULL;
	char * cipher = NULL;
	int cl_len = strlen(basic->uid ? basic->uid : "") + strlen(basic->pw ? basic->pw : "") + 5;
	int ci_len = 5 + 4 * (cl_len/3);
	if ((cleartext = (char *) HT_CALLOC(1, cl_len)) == NULL)
	    HT_OUTOFMEM("basic_credentials");
	*cleartext = '\0';
	if (basic->uid) strcpy_s(cleartext, cl_len, basic->uid);
	strcat_s(cleartext, cl_len, ":");
	if (basic->pw) strcat_s(cleartext, cl_len, basic->pw);
	...
	/* Create the credentials and assign them to the request object */
	{
	    ...
	    strcpy_s(cookie, cr_len + 1, "Basic ");
	    strcat_s(cookie, cr_len + 1, cipher);
...
PRIVATE BOOL digest_credentials (HTRequest * request, HTDigest * digest)
{
	...
	digest->nc++;
	sprintf_s(nc, sizeof(nc), "%08lx", digest->nc);
	...
	/* Create the credentials and assign them to the request object */
	{
	    ...
	    strcpy_s(cookie, cr_len + 1, "Digest ");
	    strcat_s(cookie, cr_len + 1, cleartext);
