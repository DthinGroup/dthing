...
PUBLIC char * HTParse (const char *aName, const char *relatedName, int wanted)
{
    ...
    if (wanted & PARSE_ACCESS)
        if (access) {
	    strcat_s(result, len, access);
	    if(wanted & PARSE_PUNCTUATION) strcat_s(result, len, ":");
	}
    ...
    if (wanted & PARSE_HOST)
	    ...
	    if(wanted & PARSE_PUNCTUATION) strcat_s(result, len, "//");
	    strcat_s(result, len, given.host ? given.host : related.host);
	}
    ...
    if (wanted & PARSE_PATH) {
	    ...
	    if(wanted & PARSE_PUNCTUATION) strcat_s(result, len, "/");
	    strcat_s(result, len, given.absolute);
	} else if(related.absolute) {	/* Adopt path not name */
	    strcat_s(result, len, "/");
	    strcat_s(result, len, related.absolute);
	    if (given.relative) {
		p = strchr(result, '?');	/* Search part? */
		if (!p) p=result+strlen(result)-1;
		for (; *p!='/'; p--);	/* last / */
		p[1]=0;					/* Remove filename */
		strcat_s(result, len, given.relative);		/* Add given one */
#if 0
		result = HTSimplify (&result);
#endif
	    }
	} else if(given.relative) {
	    strcat_s(result, len, given.relative);		/* what we've got */
	} else if(related.relative) {
	    strcat_s(result, len, related.relative);
	} else {  /* No inheritance */
	    strcat_s(result, len, "/");
	}
    }
	...
	if(given.fragment || related.fragment) {
		...
		if(wanted & PARSE_PUNCTUATION) strcat_s(result, len, "#");
		strcat_s(result, len, given.fragment);
	    } else if (!(given.absolute) && !(given.fragment)) {
		strcat_s(result, len, "");
	    } else {
		if(wanted & PARSE_PUNCTUATION) strcat_s(result, len, "#");
		strcat_s(result, len, given.fragment ? given.fragment : related.fragment);
	    }
	}
    HT_FREE(rel);
    HT_FREE(name);
...
PRIVATE char * HTCanon (char ** filename, char * host)
{
...
#if 0
    if (((strptr = strchr(host, '.')) == NULL || strptr >= path) && strncasecomp(host, "localhost", 9)) {
    	const char *domain = HTGetDomainName();
	    if (domain && *domain) {
            newnameSize = strlen(*filename) + strlen(domain)+2;
            if ((newname = (char *) HT_CALLOC(1, newnameSize)) == NULL) {
                HT_OUTOFMEM("HTCanon");
            }
            if (port) {
                strncpy_s(newname, newnameSize, *filename, (int) (port-*filename));
            } else {
                strncpy_s(newname, newnameSize, *filename, (int) (path-*filename));
            }
            strcat_s(newname, newnameSize, ".");
            strcat_s(newname, newnameSize, domain);
        }
    ...
    if (port) {
        if (!*(port+1) || *(port+1)=='/') {
            ...
            strncat_s(newname, newnameSize, port, (int) (path-port));
        }
    }

    if (newname) {
        char *newpath = newname+strlen(newname);
        strcat_s(newname, newnameSize, path);
        path = newpath;
...
PUBLIC char * HTRelative (const char * aName, const char * relatedName)
{
    ...
    resultSize = 3*levels + strlen(last_slash) + 4;
	if ((result = (char  *) HT_MALLOC(resultSize)) == NULL)
	    HT_OUTOFMEM("HTRelative");
	*result = '\0';
	if (!levels) strcat_s(result, resultSize, "./");
	for(;levels; levels--)strcat_s(result, resultSize, "../");
	strcat_s(result, resultSize, last_slash+1);
	if (!*result) strcat_s(result, resultSize, "./");
    }
