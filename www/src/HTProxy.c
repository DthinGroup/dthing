...
PUBLIC void HTProxy_getEnvVar (void)
{
    ...
    while (*access) {
        ...
        int len = 0;
        errno_t err = 0;
        BOOLEAN freeproxy = FALSE;

        /* Search for proxy gateways */
        if (found == NO) {
            strcpy_s(buf, sizeof(buf), *access);
            strcat_s(buf, sizeof(buf), "_proxy");

            err = _dupenv_s(&proxy, &len, buf);
            if (err != 0 || proxy == NULL) {
                proxy = NULL;
            } else {
                freeproxy = TRUE;
        ...
        if (found == NO) {
            strcpy_s(buf, sizeof(buf), "WWW_");
            strcat_s(buf, sizeof(buf), *access);
            strcat_s(buf, sizeof(buf), "_GATEWAY");

            err = _dupenv_s(&gateway, &len, buf);
    ...
    /* Search for `noproxy' directive */
    {
        ...
        err = _dupenv_s(&noproxy, &len, "no_proxy");
        if (err != 0 || noproxy == NULL) {
            noproxy = NULL;
        } else {
            freeproxy = TRUE;
        }
