...
// fwd
static Object* processEncodedAnnotation(const ClassObject* clazz, const u1** pPtr);
static bool_t skipEncodedAnnotation(const ClassObject* clazz, const u1** pPtr);
 ...
 */
static u4 readUnsignedInt(const u1* ptr, int zwidth, bool_t fillOnRight)
{
    u4 val = 0;
    int i;
 ...
 * "fillOnRight" indicates which side we want to zero-fill from.
 */
static u8 readUnsignedLong(const u1* ptr, int zwidth, bool_t fillOnRight)
{
    u8 val = 0;
    int i;
...
static Method* resolveAmbiguousMethod(const ClassObject* referrer, u4 methodIdx)
{
    ...
    if (resClass == NULL) {
        ...
        DVMTraceDbg("resolveAmbiguousMethod: unable to find class %d", methodIdx);
        return NULL;
    }
    if (dvmIsInterfaceClass(resClass)) {
        /* method is part of an interface -- not expecting that */
        DVMTraceDbg("resolveAmbiguousMethod: method in interface?");
        return NULL;
    }
 ...
 * result is wanted
 */
typedef enum AnnotationResultStyle_e {
 ...
 * will be raised.
 */
static bool_t processAnnotationValue(const ClassObject* clazz,
    const u1** pPtr, AnnotationValue* pValue,
    AnnotationResultStyle resultStyle)
{
    //Thread* self = dvmThreadSelf();
    Object* elemObj = NULL;
    bool_t setObject = FALSE;
    const u1* ptr = *pPtr;
    ...
    DVMTraceInf("----- type is 0x%02x %d, ptr=%p [0x%06x]",
    ...
    case kDexAnnotationArray:
        ...
        if (resultStyle == kAllRaw) {
            ...
            DVMTraceInf("--- annotation array, size is %u at %p", size, ptr);
            //TODO: how to alloc a new JavaLangObjecArray ?
            //newArray = dvmAllocArrayByClass(gDvm.classJavaLangObjectArray,
            //    size, ALLOC_DEFAULT);
            if (newArray == NULL) {
                DVMTraceErr("annotation element array alloc failed (%d)", size);
                return FALSE;
            }
    ...
    case kDexAnnotationNull:
        ...
        break;
    default:
        DVMTraceErr("Bad annotation element value byte 0x%02x (0x%02x)",
...
static Object* convertReturnType(Object* valueObj, ClassObject* methodReturn)
{
     ...
     */
    if (CRTL_strcmp(valueObj->clazz->descriptor, "[Ljava/lang/Object;") != 0) {
        DVMTraceErr("Unexpected src type class (%s)", valueObj->clazz->descriptor);
        return NULL;
    }
    //TODO: create gDvm.classJavaLangObject in next step
    ...
    DVMTraceInf("HEY: converting valueObj from [%s to [%s",
    ...
    if (newArray == NULL) {
        DVMTraceErr("Failed creating duplicate annotation class (%s %d)",
    ...
    if (!success) {
        DVMTraceErr("Annotation array copy failed");
        //dvmReleaseTrackedAlloc((Object*)newArray, self);
        newArray = NULL;
        goto bail;
    }
 ...
 */
static bool_t skipAnnotationValue(const ClassObject* clazz, const u1** pPtr)
{
#if 0
    const u1* ptr = *pPtr;
    u1 valueType, valueArg;
    ...
    const char* descriptor)
{
    ...
    return CRTL_strcmp(str, descriptor);
}
 ...
 */
bool_t dvmIsReflectionMethod(const Method* method)
{
    //unimplement
    return FALSE;
}
 ...
 */
bool_t dvmIsClassAnnotationPresent(const ClassObject* clazz,
        const ClassObject* annotationClazz)
{
#if 0
    const DexAnnotationSetItem* pAnnoSet = findAnnotationSetForClass(clazz);
 ...
 * Caller must call dvmReleaseTrackedAlloc().
 */
bool_t dvmGetInnerClass(const ClassObject* clazz, StringObject** pName,
    int* pAccessFlags)
{
#if 0
    const DexAnnotationItem* pAnnoItem;
 ...
 * Returns true if the annotation exists.
 */
bool_t dvmIsMethodAnnotationPresent(const ClassObject* clazz,
 ...
 * Returns true if the annotation exists.
 */
bool_t dvmIsFieldAnnotationPresent(const ClassObject* clazz,
...
static int countMethodArguments(const Method* method)
{
    ...
    return (int)CRTL_strlen(method->shorty + 1);
}
 ...
 */
bool_t dvmEncodedArrayIteratorHasNext(const EncodedArrayIterator* iterator) {
 ...
 */
bool_t dvmEncodedArrayIteratorGetNext(EncodedArrayIterator* iterator,
    ...
    if (! processed) {
        DVMTraceErr("Failed to process array element %d from %p",
