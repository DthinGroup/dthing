...
void dexStringCacheAlloc(DexStringCache* pCache, size_t length)
{
    ...
    if (length <= sizeof(pCache->buffer)) {
        ...
        pCache->value = (char*) CRTL_malloc(length);
...
char* dexStringCacheEnsureCopy(DexStringCache* pCache, const char* value) {
    if (value != pCache->value) {
        size_t length = CRTL_strlen(value) + 1;
        dexStringCacheAlloc(pCache, length);
        CRTL_memcpy(pCache->value, value, length);
    }
 ...
 */
static const DexProtoId* getProtoId(const DexProto* pProto)
{
...
/* (documented in header file) */
const char* dexProtoGetMethodDescriptor(const DexProto* pProto,  DexStringCache* pCache)
{
    ...
    for (i = 0; i < paramCount; i++) {
        ...
        length += CRTL_strlen(dexStringByTypeIdx(dexFile, idx));
    }

    length += CRTL_strlen(dexStringByTypeIdx(dexFile, protoId->returnTypeIdx));

    dexStringCacheAlloc(pCache, length);

    at = (char*) pCache->value;
    *(at++) = '(';
    ...
    *(at++) = ')';
    ...
    CRTL_strcpy(at, dexStringByTypeIdx(dexFile, protoId->returnTypeIdx));
    return pCache->value;
}
...
/* (documented in header file) */
const char* dexProtoGetParameterDescriptors(const DexProto* pProto, DexStringCache* pCache)
{
    ...
    for (;;) {
        ...
        CRTL_strcpy(at, descriptor);
        at += CRTL_strlen(descriptor);
    }
 ...
 */
static int protoCompare(const DexProto* pProto1, const DexProto* pProto2, bool_t compareReturnType)
{
    int minParam;
    int i;
    ...
    if (pProto1 == pProto2) {
        ...
        if (compareReturnType) {
            int result =
                CRTL_strcmp(dexStringByTypeIdx(dexFile1, protoId1->returnTypeIdx),
        ...
        minParam = (paramCount1 > paramCount2) ? paramCount2 : paramCount1;

        for (i = 0; i < minParam; i++) {
            u4 idx1 = dexTypeListGetIdx(typeList1, i);
            u4 idx2 = dexTypeListGetIdx(typeList2, i);
            int result =
                CRTL_strcmp(dexStringByTypeIdx(dexFile1, idx1),
 ...
 * argument types).
 */
static int protoCompareToParameterDescriptors(const DexProto* proto, const char* descriptor, bool_t expectParens)
{
#if 0
...
int dexProtoCompareToDescriptor(const DexProto* proto, const char* descriptor)
{
    ...
    int result = CRTL_strcmp(dexProtoGetReturnType(proto), returnType);
...
/* (documented in header file) */
int dexProtoCompareToParameterDescriptors(const DexProto* proto, const char* descriptors)
{
#if 0
 ...
 * caller.
 */
char* dexCopyDescriptorFromMethodId(const DexFile* pDexFile, const DexMethodId* pMethodId)
{
    DexProto proto;
 ...
 */
const char* dexGetDescriptorFromMethodId(const DexFile* pDexFile, const DexMethodId* pMethodId, DexStringCache* pCache)
{
    DexProto proto;
 ...
 */
void dexProtoSetFromMethodId(DexProto* pProto, const DexFile* pDexFile, const DexMethodId* pMethodId)
{
