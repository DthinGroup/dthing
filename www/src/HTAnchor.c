...
PUBLIC char * HTAnchor_address  (HTAnchor * me)
{
    ...
    if (me) {
        if (((HTParentAnchor *) me == me->parent) ||
            ...
            int size = 2 + strlen(me->parent->address) + strlen(((HTChildAnchor *) me)->tag);
            if ((addr = (char  *) HT_MALLOC(size)) == NULL)
                HT_OUTOFMEM("HTAnchor_address");
            sprintf_s(addr, size, "%s#%s", me->parent->address,
...
PUBLIC char * HTAnchor_expandedAddress  (HTAnchor * me)
{
	...
	if (((HTParentAnchor *) me == me->parent) ||
        ...
        int size = 2 + strlen(base) + strlen(((HTChildAnchor *) me)->tag);
	    if ((addr = (char *) HT_MALLOC(size)) == NULL)
	        HT_OUTOFMEM("HTAnchor_address");
	    sprintf_s(addr, size, "%s#%s", base, ((HTChildAnchor *) me)->tag);
	}
    }
    return addr;
}
