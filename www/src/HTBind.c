...
#else
    if (strtok_s(file, HTDelimiters, &lasts)) { /* Do we have any suffixes? */
#endif /* HT_REENTRANT */
        char *suffix;
#ifdef HT_REENTRANT
        while ((suffix=(char*)strtok_r(NULL, HTDelimiters, &lasts)) != NULL) {
#else
        while ((suffix=strtok_s(NULL, HTDelimiters, &lasts)) != NULL) {
#endif /* HT_REENTRANT */
            HTBind *suff=NULL;
            int hash;
            unsigned char * p;
            ...
            if (HTBindings[hash]) {
                ...
                while ((suff = (HTBind *) HTList_nextObject(cur))) {
                    if ((HTCaseSen && !strcmp(suff->suffix, suffix)) || !strcasecomp(suff->suffix, suffix)) {
