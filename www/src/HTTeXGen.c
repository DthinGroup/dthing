...
#define ME_BUF_SIZE (2 * BUFFER_SIZE)
...
struct _HTStructured {
	...
	char				buffer[ME_BUF_SIZE];   /* See note */
	char *				write_pointer;
	char *				line_break;
...
PRIVATE int HTTeXGen_put_character(HTStructured * me, char c)
{
    ...
    if (c == '\n') {
        ...
        *me->write_pointer++ = c;
        ...
        if (c == '$' || c == '&' || c == '%' || c == '#' || c == '{' || c == '}' || c == '_') { /* Special chars */
            *me->write_pointer++ = '\\';
            *me->write_pointer++ = c;
        } else if (c == '\\') {                   /* Special names */
            char *temp = "$\\backslash$";
            strcpy_s(me->write_pointer, me->buffer + ME_BUF_SIZE - me->write_pointer, temp);
