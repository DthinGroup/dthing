 ...
 */
ClassObject* dvmResolveClass(const ClassObject* referrer, u4 classIdx, bool_t fromUnverifiedConstant)
{
    ...
    DVMTraceInf("--- resolving class %u (referrer=%s)",
        classIdx, referrer->descriptor);
    ...
    if (resClass != NULL)
    {
        ...
        if (!fromUnverifiedConstant && IS_CLASS_FLAG_SET(referrer, CLASS_ISPREVERIFIED))
        {
            ...
            {
                DVMTraceWar("Class resolved by unexpected DEX:"
                     " %s:%p ref [%s] %s:%p",
                    referrer->descriptor, referrer->pDvmDex,
                    resClass->descriptor, resClassCheck->descriptor,
                    resClassCheck->pDvmDex);
                DVMTraceWar("(%s had used a different %s during pre-verification)",
        ...
        DVMTraceInf("##### +ResolveClass(%s): referrer=%s dex=%p ref=%d",
            resClass->descriptor, referrer->descriptor, referrer->pDvmDex, classIdx);
        ...
        DVMTraceInf("Class not found: %s", dexStringByTypeIdx(pDvmDex->pDexFile, classIdx));
...
Method* dvmResolveMethod(const ClassObject* referrer, u4 methodIdx, MethodType methodType)
{
    ...
    DVMTraceInf("--- resolving method %u (referrer=%s)", methodIdx, referrer->descriptor);
    ...
    if (dvmIsInterfaceClass(resClass))
    {
        ...
        //dvmThrowIncompatibleClassChangeErrorWithClassMessage(resClass->descriptor);
        //TODO: should throw exeception here?
        return NULL;
    }

    name = dexStringById(pDvmDex->pDexFile, pMethodId->nameIdx);
    ...
    DVMTraceInf("--- found method %d (%s.%s)",
    ...
    {
        if (!dvmIsClassInitialized(resMethod->clazz) && !dvmInitClass(resMethod->clazz))
        {
            //TODO: should throw exeception here?
            //assert(dvmCheckException(dvmThreadSelf()));
            return NULL;
    ...
    {
        DVMTraceInf("--- not caching resolved method %s.%s (class init=%d/%d)",
...
Method* dvmResolveInterfaceMethod(const ClassObject* referrer, u4 methodIdx)
{
    ...
    DVMTraceInf("--- resolving interface method %d (referrer=%s)",
    ...
    DVMTraceInf("--- found interface method %d (%s.%s)",
        methodIdx, resClass->descriptor, resMethod->name);
...
StringObject* dvmResolveString(const ClassObject* referrer, u4 stringIdx)
{
    ...
    DVMTraceInf("+++ resolving string, referrer is %s", referrer->descriptor);
